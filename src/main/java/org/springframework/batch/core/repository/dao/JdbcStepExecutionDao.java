...
public class JdbcStepExecutionDao extends AbstractJdbcBatchMetadataDao implements StepExecutionDao, InitializingBean {
	...
	public void updateStepExecution(StepExecution stepExecution) {
		...
		synchronized (stepExecution) {
            ...
            // This is a hack for a fail over demo purposes
            // -----------------------------------------------------------------------------------------------------
            int hackVersion = getJdbcTemplate().queryForInt(
                    "select version from BATCH_STEP_EXECUTION where step_execution_id = ?", stepExecution.getId() );

            if ( hackVersion != stepExecution.getVersion() ) {
                logger.info( "(!!!) Possible failover while running over several nodes" );
                logger.info( "(!!!) Trying to recover..." );
                stepExecution.setVersion( hackVersion );
            }
            // -----------------------------------------------------------------------------------------------------
